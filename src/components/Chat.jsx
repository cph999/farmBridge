...
function Chat({ userinfo, websocket }) {
    ...
    const handleAddFtiend = async (user) => {
        instance.post("/addFriend", {
            fromId: userinfo.id,
            toId: user.id,
            fromNickname: userinfo.nickname,
            toNickname: user.nickname || user.username,
    ...
    const select = (option) => {
        console.log(option);
        if (option.text === "添加好友") setAddFriendState(true);
    };
